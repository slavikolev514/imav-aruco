...
int main(int argc, char *argv[]) {
    ...
    //aruco::Dictionary dictionary = cv::aruco::getPredefinedDictionary(cv::aruco::DICT_5X5_1000);
    ...
    while(inputVideo.grab()) {
        ...
        cout << "Distance from camera: " << distance_from_cam << endl; // distance in meters

        // --- Compute angles (in degrees) ---
        //double yaw   = atan2(tvecs[0], tvecs[2]) * 180.0 / CV_PI; // left-right
        //double pitch = atan2(tvecs[1], tvecs[2]) * 180.0 / CV_PI; // up-down

        // --- Compute entire orientation ---
        //cv::Mat R;
        //cv::Rodrigues(rvecs, R);  // Convert to rotation matrix


        // TODO: make these statements actually calculate roll pitch yaw 
        //double roll  = atan2(rvecs[2].at(1), rvecs[2].at(2));
        //double pitch = atan2(-rvecs.at(2,0), sqrt(rvecs.at(2,1)*rvecs.at(2,1) + rvecs.at(2,2)*rvecs.at(2,2)));
        //double yaw   = atan2(rvecs.at(1,0), rvecs.at(0,0));

        //std::cout << "Marker ID: " << ids[i] << std::endl;
        //cout << "  Translation (x,y,z): " << tvecs << endl;
        //cout << "  Distance: " << distance << " m" << endl;
        //cout << "  Roll: " << roll << " deg" << endl;
        //cout << "  Yaw: " << yaw << " deg" << endl;
        //cout << "  Pitch: " << pitch << " deg" << endl;

        // Optionally draw axes for visualization
        //cv::aruco::drawAxes(/*image*/ frame, cameraMatrix, distCoeffs, rvecs, tvecs, markerLength * 0.5f);
